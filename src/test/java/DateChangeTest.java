...
public class DateChangeTest {
    ...
    @Test
    @DisplayName("Should Change The Date")
    public void ShouldChangeTheDate() {
    ...
    @Test
    @DisplayName("Should Set up the administrative city")
    public void ShouldSetUpAdministrativeCityGatchina() {
        var validUser = DataGenerator.Registration.generateUser("ru");
        var daysToAddForFirstMeeting = 4;
        var firstMeetingDate = DataGenerator.generateDate(daysToAddForFirstMeeting);
    ...
    @Test
    @DisplayName("Should Set Up Meeting If Name Has YO")
    public void ShouldSetUpMeetingIfNameHasYO() {
        var validUser = DataGenerator.Registration.generateUser("ru");
        var daysToAddForFirstMeeting = 4;
        var firstMeetingDate = DataGenerator.generateDate(daysToAddForFirstMeeting);
        ...
        $("[data-test-id=date] input").sendKeys(Keys.chord(Keys.SHIFT, Keys.HOME), Keys.BACK_SPACE);
        $("[data-test-id=date] input").setValue(firstMeetingDate);
        $("[data-test-id=name] input").setValue("Королёв Пётр");
        $("[data-test-id=phone] input").setValue(validUser.getPhone());
        $("[data-test-id=agreement]").click();
        $(byText("Запланировать")).click();
        $(byText("Успешно!")).shouldBe(Condition.visible, Duration.ofSeconds(15));
